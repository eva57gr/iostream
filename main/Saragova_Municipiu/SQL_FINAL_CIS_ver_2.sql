...
--SUM(CASE WHEN  C.FULL_CODE LIKE '%'||CR.CS_CUATM||'%'  THEN L.COL4 ELSE NULL END ) AS CEL_EREPORTING,
...
+ CASE WHEN  COL12 > 0 AND  COL11 IS NOT NULL THEN 1 ELSE 0  END 
...
FROM
CIS.VW_DATA_ALL D 
INNER JOIN USER_EREPORTING.VW_DATA_ALL_PRIMIT PR ON PR.CUIIO = D.CUIIO AND PR.CUIIO_VERS = D.CUIIO_VERS AND D.PERIOADA=PR.PERIOADA AND D.FORM=PR.FORM AND  D.FORM=PR.FORM AND PR.ID_SCHEMA IN ('1')
WHERE
