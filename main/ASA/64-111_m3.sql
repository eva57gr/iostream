SELECT 
    CASE 
        ...
        ) = 0 THEN 'Tabelul A nu con?ine niciun rând'
        WHEN COUNT(B.COL3) = 0 THEN 'Coloanele nu sunt egale'
        ELSE 'Exista valori comune'
    END AS rezultat
FROM
(
    SELECT 
...
LEFT JOIN (
    ...
    WHERE 
        PRIM IN ('1') AND 
        (
            SUBSTR(CODUL, 2, 2) IN ('47', '45') OR 
            SUBSTR(CODUL, 2, 3) IN ('461')
        )
) B ON B.COL3 = A.CAEM2
GROUP BY A.CNT_R, A.RIND, A.CNT, A.CAEM2
HAVING COUNT(B.COL3) = 0;
