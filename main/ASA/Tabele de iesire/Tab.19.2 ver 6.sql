...
-- THEN    SUBSTR(D.CAEM2,2,4)  ELSE MAX(CASE WHEN  D.RIND IN ('8') AND D.CAPITOL IN (1129) THEN D.COL31 END ) END 
-- 
-- AS CAEM2_ACTUALIZAT,
 MAX(CASE WHEN  D.RIND IN ('8') AND D.CAPITOL IN (1129) THEN D.COL31 END) CAEM2_CIS,
--
 --D.CAEM2  AS CAEM2,
 SUBSTR(D.CAEM2,2,4)  AS CAEM2,
...
--AND  D.CUIIO=40856685
 ...
 D.CUIIO_VERS,
 D.CAEM2
-- CASE WHEN  D.RIND IN ('8') AND D.CAPITOL IN (1129) THEN D.COL31 END
 
 
 HAVING
  CIS2.NVAL(SUM(CASE WHEN D.CAPITOL IN (100)  AND D.RIND IN ('1') THEN  D.COL1 ELSE 0 END))
  + CIS2.NVAL(SUM(CASE WHEN D.CAPITOL IN (100)  AND D.RIND IN ('5') THEN  D.COL1 ELSE 0 END))
    > 0
AND 
...
PRIM                
 ...
 --SUBSTR( C.CODUL,2,4) = TRIM(DD.CAEM2_ACTUALIZAT)              
 
 SUBSTR(C.CODUL,2,4)  = CASE WHEN DD.CAEM2_CIS IS NULL THEN DD.CAEM2 ELSE DD.CAEM2_CIS END   
               INNER JOIN (
