...
--
 ...
 CIS2.NVAL(COUNT(DISTINCT (CASE WHEN D.RIND  IN ('130') AND D.CAPITOL IN (1181) AND CIS2.NVAL(D.COL1)=1 AND C.FULL_CODE LIKE '%'||CD.CUATM||';%' THEN D.CUIIO END))) AS COL1, 
 CIS2.NVAL(COUNT(DISTINCT (CASE WHEN D.RIND  IN ('140') AND D.CAPITOL IN (1181) AND CIS2.NVAL(D.COL1)=1 AND C.FULL_CODE LIKE '%'||CD.CUATM||';%' THEN D.CUIIO END))) AS COL2, 
 SUM(CASE WHEN D.RIND  IN ('141') AND D.CAPITOL IN (1181) AND C.FULL_CODE LIKE '%'||CD.CUATM||';%' THEN D.COL1 ELSE NULL END) AS COL3,
 CIS2.NVAL(COUNT(DISTINCT (CASE WHEN D.RIND  IN ('150') AND D.CAPITOL IN (1181) AND CIS2.NVAL(D.COL1)=1 AND C.FULL_CODE LIKE '%'||CD.CUATM||';%' THEN D.CUIIO END))) AS COL4
