    ...
    -- COUNT (DISTINCT CASE WHEN  MC.CAPITOL IN (405)  AND MR.RIND NOT IN ('1','-')  AND CIS2.NVAL(D.COL4) > 0  THEN D.CUIIO  END)   AS COL1,
     ...
     COUNT (DISTINCT CASE WHEN  MC.CAPITOL IN (405)  AND MR.RIND NOT IN ('1','-')  AND D.COL33 IS NOT NULL  THEN D.CUIIO  END)   AS COL1 
     ...
     FROM DATA_ALL D
       ...
       INNER JOIN VW_CLS_CLASS_ITEM TT ON (TT.CLASS_CODE IN ('TARI_ISO')   AND   (TT.ITEM_CODE=D.COL33))
        ...
        ------------------------------------------------------------------------------   
WHERE 
 D.PERIOADA IN (1063,1062,1061,1060) AND 
  (D.FORM =:pFORM) AND
 ...
 -- (:pID_MDTABLE =:pID_MDTABLE) AND
  ...
  D.FORM IN (44)  AND
  MC.CAPITOL IN (405)  
--  AND TTT.ITEM_CODE NOT IN ('444','555')
--  AND D.CUIIO = 43044 
  GROUP BY
  TTT.ITEM_CODE,
  TTT.SHOW_ORDER,
