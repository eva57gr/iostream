...
--------------------------------------------------------------------------------
 SELECT   
    ...
    '0' AS NR_SECTIE2,
    '0' AS NUME_SECTIE2,
    -- CASE WHEN INSTR(TO_CHAR(AA.COL1)||'~'||ROWNUM,'.',1) = 1 THEN 0||AA.COL1||'~'||ROWNUM ELSE AA.COL1||'~'||ROWNUM END AS NR_ROW,
     ROWNUM  AS NR_ROW,
     AA.ORDINE  AS ORDINE,
    '0000' AS DECIMAL_POS,
     AA.NUME_ROW AS NUME_ROW,
     ROUND(AA.COL1,1) AS COL1,
     ROUND(AA.COL2,1) AS COL2,
     ROUND(AA.COL3,1) AS COL3,
     ROUND(AA.COL4,1) AS COL4
...
--
 SELECT
     ...
     ROUND(SUM(CASE WHEN VC.FUlL_CODE LIKE '%'||CD.CUATM||';%' AND  D.RIND IN ('000') THEN CIS2.NVAL(D.COL1)ELSE NULL END),1)  
     AS COL1,
     
     
     ROUND(SUM(CASE WHEN VC.FUlL_CODE LIKE '%'||CD.CUATM||';%' AND  D.RIND IN ('999')  THEN CIS2.NVAL(D.COL1) ELSE 0 END),1) 
     AS COL2,
     
         ROUND(SUM(CASE WHEN VC.FUlL_CODE LIKE '%'||CD.CUATM||';%' AND  D.RIND IN ('000') THEN CIS2.NVAL(D.COL2) ELSE NULL END),1)  
     AS COL3,
     
     
     ROUND(SUM(CASE WHEN VC.FUlL_CODE LIKE '%'||CD.CUATM||';%' AND  D.RIND IN ('999')  THEN CIS2.NVAL(D.COL2)  ELSE 0 END),1) 
     AS COL4
