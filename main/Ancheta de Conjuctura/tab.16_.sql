...
A.RIND
HAVING 
COUNT(DISTINCT CASE WHEN A.RIND IN ('16.2')  THEN A.CUIIO END) > 0) L LEFT JOIN (
...
HAVING 
COUNT(DISTINCT CASE WHEN A.RIND IN ('16.2') AND (TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%' OR (ROUND(BS.PERS_IT) >= TR.PERS_INIT AND ROUND(BS.PERS_IT) <= TR.PERS_FINAL)) THEN A.CUIIO END) > 0
