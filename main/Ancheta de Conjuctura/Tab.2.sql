...
SELECT
  ...
  TR.RIND,
--  SUBSTR(A.CAEM2,1,1),
  COUNT(CASE WHEN A.RIND IN ('2.1') AND (TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%' OR (BS.PERS_IT >= TR.PERS_INIT AND BS.PERS_IT <= TR.PERS_FINAL)) THEN A.CUIIO END) AS COL1,
  COUNT(CASE WHEN A.RIND IN ('2.2') AND (TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%' OR (BS.PERS_IT >= TR.PERS_INIT AND BS.PERS_IT <= TR.PERS_FINAL)) THEN A.CUIIO END) AS COL2,
  COUNT(CASE WHEN A.RIND IN ('2.3') AND (TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%' OR (BS.PERS_IT >= TR.PERS_INIT AND BS.PERS_IT <= TR.PERS_FINAL)) THEN A.CUIIO END) AS COL3
FROM
(SELECT
  D.ANUL,
  D.CUIIO,
...
WHERE
  ...
  D.RIND IN ('2.1','2.2','2.3')
) A LEFT JOIN
  X_BAZA_SONDAJ BS ON (A.CUIIO=BS.CUIIO AND BS.ANUL=2021)
  CROSS JOIN 
  (
    SELECT 1 AS NR_TABLE, 'CAEM2' AS TABLE_DENUMIRE, NULL AS PERS_INIT, NULL AS PERS_FINAL, 'B+C+D+E+F+G+H+I+J+L+M+N+R+S' AS RIND, 'Total activitati' AS RIND_DENUMIRE, '1' AS ORDINE FROM DUAL UNION
    SELECT 1 AS NR_TABLE, 'CAEM2' AS TABLE_DENUMIRE, NULL AS PERS_INIT, NULL AS PERS_FINAL, 'B+C+D+E' AS RIND, 'Industrie (B+C+D+E)' AS RIND_DENUMIRE, '2' AS ORDINE FROM DUAL UNION
    SELECT 1 AS NR_TABLE, 'CAEM2' AS TABLE_DENUMIRE, NULL AS PERS_INIT, NULL AS PERS_FINAL, 'F' AS RIND, 'Conctruc?ie (F)' AS RIND_DENUMIRE, '3' AS ORDINE FROM DUAL UNION
    SELECT 1 AS NR_TABLE, 'CAEM2' AS TABLE_DENUMIRE, NULL AS PERS_INIT, NULL AS PERS_FINAL, 'G+H+I+J+L+M+N+R+S' AS RIND, 'Servicii (G+H+I+J+L+M+N+R+S)' AS RIND_DENUMIRE, '4' AS ORDINE FROM DUAL UNION
...
SELECT
  ...
  CC.CS_CUATM AS RIND,
--  SUBSTR(A.CAEM2,1,1),
  COUNT(CASE WHEN A.RIND IN ('2.1') AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' THEN A.CUIIO END) AS COL1,
  COUNT(CASE WHEN A.RIND IN ('2.2') AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' THEN A.CUIIO END) AS COL2,
  COUNT(CASE WHEN A.RIND IN ('2.3') AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' THEN A.CUIIO END) AS COL3
FROM
(SELECT
  D.ANUL,
  D.CUIIO,
...
WHERE
  ...
  INNER JOIN VW_CL_CUATM C ON(A.CUATM = C.CODUL)
  CROSS JOIN 
  (
    SELECT
      DECODE(CS.CODUL, 
