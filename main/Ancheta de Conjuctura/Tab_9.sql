...
SELECT
  ...
  TR.RIND,
--  SUBSTR(A.CAEM2,1,1),
  COUNT(DISTINCT CASE WHEN A.RIND IN ('9') AND (TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%' OR (ROUND(BS.PERS_IT) >= TR.PERS_INIT AND ROUND(BS.PERS_IT) <= TR.PERS_FINAL)) THEN A.CUIIO END) AS COL1,
  COUNT(DISTINCT CASE WHEN A.RIND IN ('9') AND (TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%' OR (ROUND(BS.PERS_IT) >= TR.PERS_INIT AND ROUND(BS.PERS_IT) <= TR.PERS_FINAL)) THEN A.CUIIO END) AS COL2
FROM
(SELECT
  D.ANUL,
  D.CUIIO,
...
SELECT
  ...
  CC.CS_CUATM AS RIND,
--  SUBSTR(A.CAEM2,1,1),
  COUNT(DISTINCT CASE WHEN A.RIND IN ('9') AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' THEN A.CUIIO END) AS COL1,
  COUNT(DISTINCT CASE WHEN A.RIND IN ('9') AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' THEN A.CUIIO END) AS COL2
FROM
(SELECT
  D.ANUL,
  D.CUIIO,
...
(SELECT
  ...
  A.CAEM2 AS RIND,
  COUNT(DISTINCT CASE WHEN A.RIND IN ('9')  AND C.FULL_CODE LIKE '%'||CC.CODUL||';%' THEN A.CUIIO END) AS COL1,
  COUNT(DISTINCT CASE WHEN A.RIND IN ('9')  AND C.FULL_CODE LIKE '%'||CC.CODUL||';%' THEN A.CUIIO END) AS COL2
FROM
(SELECT
  D.ANUL,
  D.CUIIO,
