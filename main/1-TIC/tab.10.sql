...
--)
 ...
 SELECT
  ...
  CIS2.NVAL(COUNT(DISTINCT (CASE WHEN D.RIND  IN ('610') AND D.CAPITOL IN (1186) AND C.FULL_CODE LIKE '%'||CD.CUATM||';%' AND CIS2.NVAL(D.COL1)>0 THEN D.CUIIO END))) AS COL1,
  CIS2.NVAL(COUNT(DISTINCT (CASE WHEN D.RIND  IN ('620') AND D.CAPITOL IN (1186) AND C.FULL_CODE LIKE '%'||CD.CUATM||';%' AND CIS2.NVAL(D.COL1)>0 THEN D.CUIIO END))) AS COL2,
