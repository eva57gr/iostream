...
-- ============================================================================
  SELECT
    '01' AS RIND,
    'Autorizatiile de construire eliberate pentru toate cladirile' AS DENUMIRE,
    0 AS ORDINE,
    SUM(D.COL1) AS COL1,
    SUM(D.COL2) AS COL2
  FROM
  ...
  WHERE
    D.FORM = 15 AND
    D.CAPITOL = 1000 AND
    D.RIND IN('01', '03', '04', '05', '06', '07') AND
    RR.TIP_LOCAL = 1
  ...
  FROM
    CIS2.VW_DATA_ALL D INNER JOIN
    CIS2.MD_RIND R ON(D.ID_MD=R.ID_MD) INNER JOIN 
    CIS2.RENIM RR ON (D.CUIIO = RR.CUIIO AND D.CUIIO_VERS = RR.CUIIO_VERS)
  WHERE
    D.FORM = 15 AND
    D.CAPITOL = 1000 AND
    RR.TIP_LOCAL = 1 AND
