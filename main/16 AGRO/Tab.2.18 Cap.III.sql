...
SELECT 
  ...
  NVAL(SUM(D.COL6)) AS COL6,
  NVAL(SUM(D.COL7)) AS COL7,
  NVAL(SUM(D.COL8)) AS COL8,
  NVAL(SUM(D.COL9)) AS COL9,
  NVAL(SUM(D.COL10)) AS COL10,
  NVAL(SUM(D.COL11)) AS COL11,
  NVAL(SUM(D.COL12)) AS COL12,
  NVAL(SUM(D.COL13)) AS COL13,
  NVAL(SUM(D.COL14)) AS COL14,
  NVAL(SUM(D.COL15)) AS COL15,
  NVAL(SUM(D.COL16)) AS COL16
FROM 
  VW_DATA_ALL D
  INNER JOIN RENIM RE ON (D.CUIIO=RE.CUIIO AND D.CUIIO_VERS=RE.CUIIO_VERS)  
  INNER JOIN VW_CL_CUATM CC ON(D.CUATM = CC.CODUL)
